...
app.post('/', (req, res) => {
	Game
		...
		.then((obj) => {
			User
				.create({username: req.body.username, increase: 29, gameid: obj._id})
				.then((obj) => {
...
app.post('/join', (req, res) => {
	User
		.create({ username: req.body.username, increase: 29, gameid: req.body.id})
		.then((obj) =>{
			res.redirect(`/game/${obj.gameid}?username=${obj.username}`)
		})
})
  ...
  const id = socket.handshake.headers.referer.split('/').slice(-1)[0].split('?')[0];
		...
		User
			...
			.then((user) => {
				...
				User
					.find({gameid: id})
					.then(users => {
						io.to(id).emit('update game', {users: users})
					})
			})
	});
	...
	socket.on('player moved', ({player}) => {
		User
			...
			.then((user) => {
				io.to(id).emit('update player', {player: user});
				User
					.find({gameid: id})
					.then(users => {
						users.forEach((user) => {
